...
export async function pickImages(): Promise<ImageItem[]> {
  ...
  return result.assets.map((asset) => ({
    id: asset.assetId ?? `img-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    uri: asset.uri,
    width: asset.width,
    height: asset.height,
  }));
...
export async function rotateImage(image: ImageItem, direction: "left" | "right"): Promise<ImageItem> {
...
export async function cropImage(image: ImageItem, insetPercent: number): Promise<ImageItem> {
  const { width, height } = image.width && image.height ? { width: image.width, height: image.height } : await getSize(image.uri);
  const inset = Math.min(Math.max(insetPercent, 0), 45);
  const dx = Math.floor((width * inset) / 100);
  const dy = Math.floor((height * inset) / 100);
  const manipulated = await ImageManipulator.manipulateAsync(
    image.uri,
    [{ crop: { originX: dx, originY: dy, width: width - dx * 2, height: height - dy * 2 } }],
    { compress: 1, format: ImageManipulator.SaveFormat.PNG }
  );
